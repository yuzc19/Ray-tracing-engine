...
Material *SceneParser::parseMaterial() {
  ...
  auto *answer =
      new Material(color, specularColor, shininess, emission, refr, type);
  answer->texture.image = stbi_load(textureFile, &answer->texture.w,
                                    &answer->texture.h, &answer->texture.c, 0);
  return answer;
}
